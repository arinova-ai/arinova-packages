...
export async function main() {
  const [allowBody, existingBody, packageBody] = await Promise.all([
    readFile(ALLOW, "utf8"),
  ...
  const selected = selectEntries(feeds, allowlisted, ENTRIES_PER_SOURCE);
  const existing = parseToml(existingBody);
  const entriesChanged = !entriesEqual(existing.entries ?? [], selected);
