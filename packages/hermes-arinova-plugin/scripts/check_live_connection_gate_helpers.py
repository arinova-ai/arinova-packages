...
# Executable credential and live-probe cases.

import tempfile
from pathlib import Path


def assert_invalid_probe_case(
    hermes_home: Path,
    fake_hermes_root: Path,
    *,
    slug: str,
    option: str,
    value: str,
    ...
    marker = hermes_home / f"{slug}-disconnect.txt"
    process = run_live(
        "--hermes-root",
        str(fake_hermes_root),
        option,
    ...
    assert_failed(process, expected_error)
    assert_disconnected(marker, label)
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="hermes-arinova-live-gate-") as tmp:
        ...
        for slug, value, expected_error, label in (
            (
                "bad-task-update-json",
                "{bad json",
                "SDK sendTaskUpdate() probe JSON argument could not be parsed",
                "bad sendTaskUpdate JSON live smoke",
            ),
            (
                "bad-task-update-payload",
                "[]",
                "SDK sendTaskUpdate() probe payload must be a JSON object",
                "bad sendTaskUpdate payload live smoke",
            ),
        ):
            assert_invalid_probe_case(
                hermes_home,
                fake_hermes_root,
                slug=slug,
                option="--send-task-update-json",
                value=value,
        ...
        invalid_probe_cases = (
            (
                "bad-task-update-started",
                "--send-task-update-json",
                '{"status":"started"}',
                "SDK sendTaskUpdate() probe payload must match TaskUpdateData",
                "bad sendTaskUpdate started live smoke",
            ),
            (
                "bad-task-update-unknown",
                "--send-task-update-json",
                '{"status":"started","task":"boot","durationMs":1}',
                "SDK sendTaskUpdate() probe payload must match TaskUpdateData",
                "bad sendTaskUpdate unknown field live smoke",
            ),
            (
                "bad-tool-report-json",
                "--report-tool-call-json",
                "{bad json",
                "SDK reportToolCall() probe JSON argument could not be parsed",
                "bad reportToolCall JSON live smoke",
            ),
            (
                "bad-tool-report-payload",
                "--report-tool-call-json",
                "[]",
                "SDK reportToolCall() probe payload must be a JSON object",
                "bad reportToolCall payload live smoke",
            ),
        )
        for slug, option, value, expected_error, label in invalid_probe_cases:
            assert_invalid_probe_case(
                hermes_home,
                fake_hermes_root,
                slug=slug,
